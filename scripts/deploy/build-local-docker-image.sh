...
###
 # @Author: shen
 # @Date: 2025-12-17 13:29:28
 # @LastEditors: shen
 # @LastEditTime: 2025-12-18 10:44:52
 # @Description: 
### 
...
function save_image() {
	# save image
	docker save -o ../../${IMAGE_NAME}.tar ${IMAGE_NAME} || ERROR="save_image failed"
}
...
if [[ ${ERROR} == "" ]]; then
	echo "Info: Saveing docker image" | tee -a ${LOG_FILE}
	save_image 1>> ${LOG_FILE} 2>> ${LOG_FILE}
fi

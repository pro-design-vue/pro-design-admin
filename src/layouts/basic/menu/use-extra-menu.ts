...
import { findRootMenuByPath } from '@/shared/utils'
...
function useExtraMenu(useRootMenus?: ComputedRef<MenuRecordRaw[]>) {
  ...
  const menus = computed(() => (useRootMenus?.value ?? authStore.accessMenus) as any)
  ...
  const handleMixedMenuSelect = async (menu: MenuRecordRaw) => {
    ...
    if (!willOpenedByWindow(menu.key)) {
      extraMenus.value = _extraMenus ?? []
      extraActiveMenu.value = menu.parents?.[parentLevel.value] ?? menu.key
      sidebarExtraVisible.value = hasChildren
    }

    if (!hasChildren) {
      await navigation(menu.key)
    } else if (preferences.sidebar.autoActivateChild) {
      await navigation(
        defaultSubMap.has(menu.key) ? (defaultSubMap.get(menu.key) as string) : menu.key,
      )
    }
  }
  ...
  const handleSideMouseLeave = () => {
    ...
    extraActiveMenu.value = rootMenuPath ?? findMenu?.key ?? ''
    extraMenus.value = rootMenu?.children ?? []
  }

  const handleMenuMouseEnter = (menu: MenuRecordRaw) => {
    if (!preferences.sidebar.expandOnHover) {
      const { findMenu } = findRootMenuByPath(menus.value, menu.key)

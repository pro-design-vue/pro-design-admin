...
function useMixedMenu() {
   ...
   * 全部菜单（树形结构）
   */
  const menus = computed(() => authStore.accessMenus)
  /**
   * 头部菜单
   */
  ...
  const handleMenuSelect = (key: string, mode?: string) => {
    if (!needSplit.value || mode === 'inline') {
      navigation(key)
      return
    }
    const rootMenu = menus.value.find((item) => item.key === key)
    const _splitSideMenus = rootMenu?.children ?? []

    if (!willOpenedByWindow(key)) {
      rootMenuPath.value = rootMenu?.key ?? ''
    ...
    if (_splitSideMenus.length === 0) {
      ...
      navigation(
        defaultSubMap.has(rootMenu.key)
          ? (defaultSubMap.get(rootMenu.key) as string)
          : rootMenu.key,
      )
    }
  }
  ...
  function calcSideMenus(path: string = route.path) {
    ...
    if (!rootMenu) {
      rootMenu = menus.value.find((item) => item.key === path)
    }

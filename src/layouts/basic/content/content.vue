...
import { MicroRouterView } from '../../micro'
...
const loadedRouteMap = ref<Set<string>>(new Set())
...
watchEffect(() => {
  if (!loadedRouteMap.value.has(route.name as string)) {
    loadedRouteMap.value.add(route.name as string)
  }
})
</script>
...
<template>
  <div class="layout-content">
    <IFrameRouterView :loadedRouteMap="loadedRouteMap" />
    <MicroRouterView :loadedRouteMap="loadedRouteMap" />
    <RouterView v-slot="{ Component, route }">
      <Transition v-if="getEnabledTransition" :name="getTransitionName()" appear mode="out-in">
        <KeepAlive v-if="keepAlive" :exclude="getExcludeCachedTabs" :include="getCachedTabs">
          <component
            :is="transformComponent(Component, route)"
            v-if="renderRouteView"
            v-show="!(route.meta.iframeSrc || route.meta.microUrl)"
            :key="route.fullPath"
          />
        </KeepAlive>

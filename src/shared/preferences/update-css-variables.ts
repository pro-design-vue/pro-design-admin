...
import { generatorColorVariables } from '@/shared/color'
import { updateCSSVariables as executeUpdateCSSVariables } from '@/shared/utils'
...
function updateCSSVariables(preferences: Preferences) {
  ...
  if (Reflect.has(theme, 'radius')) {
    ...
    document.documentElement.style.setProperty(`--${prefixCls}-radius`, `${radius}px`)
  }
}

/**
 * 更新主要的 CSS 变量
 * @param  preference - 当前偏好设置对象，它的颜色值将设置为 CSS 变量。
 */
function updateMainColorVariables(preference: Preferences) {
  if (!preference.theme) {
    return
  }
  const prefixCls = preference?.app?.prefixCls || DEFAULT_NAMESPACE
  ...
  // 要设置的 CSS 变量映射
  const colorMappings = {
    [`--${prefixCls}-green-500`]: `--${prefixCls}-success`,
    [`--${prefixCls}-primary-500`]: `--${prefixCls}-primary`,
    [`--${prefixCls}-red-500`]: `--${prefixCls}-destructive`,
    [`--${prefixCls}-yellow-500`]: `--${prefixCls}-warning`,
  }

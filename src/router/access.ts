...
import { VITE_APP_NAME } from '@/shared/constants'
...
function convertRoutes(
  ...
  return mapTree(menusTree, (node) => {
    ...
    const { component, name, microUrl, iframeSrc, children } = node
    const validChildren = children?.filter((item) => !item?.hideInMenu)
    ...
    if (!(iframeSrc || microUrl)) {
      ...
      if (component && layoutMap[component]) {
        ...
        if (!validChildren?.length && !isHttpUrl(route.path)) {
    ...
    const resultChildren = node.hideChildrenInMenu ? [] : (validChildren ?? [])
    ...
    if (resultChildren.length > 0) {
      ...
      if (route.redirect) {
        ...
        redirectChild = child || redirectChild
      }
      route.redirect =
        redirectChild.microUrl && VITE_APP_NAME !== redirectChild.appName
          ? `/${redirectChild.appName}${redirectChild.path}`
          : redirectChild.path
    }
...
async function generateRoutes(options: GenerateMenuAndRoutesOptions): Promise<RouteRecordRaw[]> {
  ...
  try {
    ...
    const resultRoutes = convertRoutes(menusTree, localeRouteMap, layoutMap, normalizePageMap)
